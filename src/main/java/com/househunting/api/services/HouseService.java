...
public class HouseService {
    ...
    public House createHouse(HouseRequest request, MultipartFile file, String token) {
        try {
            ...
            var house = House.builder()
                    ...
                    .addedBy(jwtService.extractUsername(token))
                    .build();
    ...
    public Object getHousesForLoggedUser(String email) {

        return houseRepository.findByAddedBy(email);
    }


    public void addToHouseUsersWishlist(House house, User user) {
        house.getUsersWishlist().add(user);
        houseRepository.save(house);
    }

    // }

    // public House updateHouse(Long id, Map<String, Object> updates, MultipartFile
    // file) {
    // House houseToUpdate = houseRepository.findById(id)
    // .orElseThrow(() -> new RuntimeException("House not found with ID: " + id));
    ...
    // public House updateHouse(Long id, MultipartFile file, String title, String
